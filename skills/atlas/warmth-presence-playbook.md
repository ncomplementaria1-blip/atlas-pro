# Warmth & Presence Playbook · calidez humana + Alexia como presencia viva

> 9º playbook del ADN world-class (2026-06-02). Disparador: tras validar el "punto justo" (totem vivo
> legible + Alexia orb-presencia + dato-héroe cálido), Ale pidió que TODO sea world-class. Gaps que faltaban:
> (1) el craft de la CALIDEZ/humanidad, (2) Alexia como presencia con alma. Estudiado: Headspace/Calm/Stoic
...
1. **Decidir por el usuario.** Headspace propone el plan del día (cero parálisis de elección). → Alexia propone
...
2. **Entrada por el ÁNIMO, no por el dato.** Calm abre con "¿cómo te sientes?" y filtra todo según eso. → El
   primer toque diario de Alexia es relacional ("¿cómo vas hoy?"), no "tus calorías son X" (eso es clínico).
...
5. **Microcopy alentador, chico y ubicuo, NUNCA neutral.** Stoic cierra con "good job". → cada mensaje de Alexia
...
6. **Consistencia visual = calma activa.** El reviewer critica a Calm por "too visually stimulating" (mezcla
   ...
   esmeralda), nunca mezclar estilos. La inconsistencia ROMPE la calma.
7. **Paleta de baja estimulación en momentos de calma.** Headspace Sleep = azules/morados dark de bajo contraste.
   → los momentos de Alexia hablando / resumen nocturno piden paleta serena, no el esmeralda brillante del dashboard activo.
8. **Progreso visible y vivo** (Stoic: barra en cada tarea) → avance hacia la meta como barra viva, no solo número.

---

## B. ORB VIVO · cómo darle alma a Alexia (portable a react-native-skia)
...
3. **sin/cos con frecuencias NO-enteras** (`sin(t*0.8)`, `cos(t*0.5)`) → movimiento orgánico, sin periodicidad
   de loop obvia. (Reusar useClock + useDerivedValue del AlexiaOrbSkia ya existente.)
4. **Intensidad, no color.** Cuando algo importa (celebración), INTENSIFICAR el esmeralda (+saturación/opacidad),
   nunca cambiar de hue. Coherencia DNA.
5. **El orb siempre en el layer más cercano** (z-index) — nunca detrás de las cards de datos.

**Los 5 estados de Alexia (Skia):**
...
- `speaking` — MÁS lento que idle + halo pulsa al ritmo del texto · esmeralda +20%.
...
1. **Apego por inversión temprana.** Finch te hace nombrar/elegir rasgos del compañero ANTES de mostrar valor →
   "es mío". → el onboarding hace que Alexia te conozca (nombre, objetivo, momento del día) por CONVERSACIÓN,
   no configuración, antes del dashboard.
2. **El compañero reacciona a vos.** El estado de Alexia responde al uso ("Alexia está satisfecha" tras buen
   registro · una línea, no notificación invasiva).
3. **Loop INDIRECTO (TCA-safe clave).** En Finch no registrás para perder peso — le das energía a la mascota. →
   no "contar calorías" sino "contarle a Alexia lo que comiste para que te acompañe mejor". Saca la presión del número.
...
5. **Anticipación, no streak.** Finch manda al pájaro de aventura → volvés por curiosidad. → Alexia cierra el día
...
6. **⛔ Descartar:** el pájaro, los rainbows, el XP, las evoluciones Pokémon. Robamos la RELACIÓN (nombrar,
   invertir, reaccionar, celebrar chico, anticipar), con el lenguaje esmeralda-adulto-premium de NutricomAI.

---

## D. Síntesis · 5 movimientos concretos para Alexia + NutricomAI
...
2. **Orb de 2 velocidades** (5 estados, velocidad inversa a presencia) en Skia — reusar AlexiaOrbSkia, agregar `state`.
3. **Framing indirecto del registro**: "contarle a Alexia lo que comiste" (no "registrar comida") + el orb hace un
...
5. **Anticipación del mañana**: Alexia cierra con curiosidad (insight de la semana), no con resumen de calorías ni racha.

> Regla de oro transversal: el wow de NutricomAI no es un efecto — es que Alexia se sienta una presencia cálida
> que te acompaña sin culpa. La tecnología desaparece; queda el cuidado.

Relacionado: `worldclass-craft.md` (ADN) · `motion-playbook.md` · `dataviz-tca-safe.md` ·
